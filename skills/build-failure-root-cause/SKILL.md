...
Use this skill when a build fails and the first actionable compiler or bundler error must be found. It turns the request into a repeatable workflow with clear checks, outputs, and validation.
...
npm run build
python -m build
rg "error|failed|Cannot find|Traceback"
```
...
```markdown
## Build Failure Root Cause Result
...
Produce the first actionable build failure and fix target.
