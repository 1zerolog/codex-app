...
description: Use when a repository needs local setup discovered and documented.
---
...
Use this skill when a repository needs local setup discovered and documented. It turns the request into a repeatable workflow with clear checks, outputs, and validation.
...
```bash
rg --files | rg "package.json|pyproject.toml|requirements.txt|Makefile"
git status --short
```
...
Produce a local setup recipe and any blockers.
