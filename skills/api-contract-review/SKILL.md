...
description: Use when an API request, response, schema, or integration contract needs review before implementation.
---
...
Use this skill when an API request, response, schema, or integration contract needs review before implementation. It turns the request into a repeatable workflow with clear checks, outputs, and validation.
...
```bash
rg "openapi|swagger|zod|yup|schema|router|endpoint"
rg "status\(|json\(|throw|error"
```
...
Produce a contract review with compatibility notes.
