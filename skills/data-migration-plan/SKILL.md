...
description: Use when a database, file format, or persistent state migration needs a safe plan.
---
...
Use this skill when a database, file format, or persistent state migration needs a safe plan. It turns the request into a repeatable workflow with clear checks, outputs, and validation.
...
```bash
rg "migration|migrate|schema|alter table|create table"
rg "rollback|down\(|revert"
```
...
Produce a migration plan with verification and rollback.
