...
description: Use when Codex needs help with keeping documentation aligned with current code and behavior.
---
...
Use this skill when Codex needs to handle keeping documentation aligned with current code and behavior. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
```bash
rg "install|quickstart|usage|setup|deprecated" README.md docs
git diff --name-only
```
...
Produce updated documentation guidance.
