...
description: Use when Codex needs help with checking package metadata and publish readiness.
---

# Package Release Desk Skill

## Purpose

Use this skill when Codex needs to handle checking package metadata and publish readiness. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
```bash
git status --short
rg "version|license|publish|build" package.json pyproject.toml
```
...
Produce a package release go/no-go.
