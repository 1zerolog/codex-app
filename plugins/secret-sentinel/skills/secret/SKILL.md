---
name: secret
description: Use when Codex needs help with scanning diffs and repos for sensitive files and secrets.
---

# Secret Sentinel Skill

## Purpose

Use this skill when Codex needs to handle scanning diffs and repos for sensitive files and secrets. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
```bash
rg "AKIA|BEGIN .*PRIVATE|token|secret|password|api[_-]?key" -S
git status --short
```
...
Produce a secrets exposure report.
