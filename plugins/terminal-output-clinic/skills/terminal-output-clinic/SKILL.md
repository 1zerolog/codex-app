---
name: terminal-output-clinic
description: Use when Codex needs help with summarizing command output into errors and next actions.
---

# Terminal Output Clinic Skill

## Purpose

Use this skill when Codex needs to handle summarizing command output into errors and next actions. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
```bash
rg "error|failed|warning|traceback|exception"
git status --short
```
