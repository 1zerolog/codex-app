...
description: Use when Codex needs help with reviewing keyboard flow, labels, semantics, and contrast.
---

# Accessibility Review Lab Skill

## Purpose

Use this skill when Codex needs to handle reviewing keyboard flow, labels, semantics, and contrast. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
```bash
rg "aria-|role=|tabIndex|alt=|label"
rg "button|input|select|textarea|a href"
```
...
```markdown
## Accessibility Review Lab Result
...
Produce an accessibility review.
