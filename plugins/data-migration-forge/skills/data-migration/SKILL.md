---
name: data-migration
description: Use when Codex needs help with planning data and schema migrations safely.
---

# Data Migration Forge Skill

## Purpose

Use this skill when Codex needs to handle planning data and schema migrations safely. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
```bash
rg "migration|schema|alter table|create table"
rg "rollback|down\(|revert|drop|truncate"
```
...
Produce a migration plan with verification.
