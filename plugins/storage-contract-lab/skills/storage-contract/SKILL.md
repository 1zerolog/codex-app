...
description: Use when Codex needs help with reviewing persistence contracts and storage lifecycle.
---

# Storage Contract Lab Skill

## Purpose

Use this skill when Codex needs to handle reviewing persistence contracts and storage lifecycle. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
```bash
rg "localStorage|sessionStorage|cache|sqlite|writeFile|readFile|storage"
rg "persist|serialize|deserialize"
```
...
Produce a storage contract review.
