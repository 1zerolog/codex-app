...
description: Use when Codex needs help with designing wrappers around external APIs.
---

# Third Party API Lab Skill

## Purpose

Use this skill when Codex needs to handle designing wrappers around external APIs. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
```bash
rg "fetch|axios|requests|httpx|rate|timeout|retry"
rg "client|adapter|provider|api"
```
