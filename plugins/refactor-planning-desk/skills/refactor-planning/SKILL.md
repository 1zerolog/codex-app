...
description: Use when Codex needs help with planning refactors without mixing behavior changes.
---

# Refactor Planning Desk Skill

## Purpose

Use this skill when Codex needs to handle planning refactors without mixing behavior changes. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
```bash
rg "TODO|FIXME|duplicate|legacy"
git diff --stat
```
...
Produce a scoped refactor plan.
