---
name: backend-service-doctor
description: Use when Codex needs help with diagnosing backend service behavior, failures, and boundaries.
---

# Backend Service Doctor Skill

## Purpose

Use this skill when Codex needs to handle diagnosing backend service behavior, failures, and boundaries. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
- entry points and service layers.
...
```bash
rg "server|app|router|controller|service"
rg "db|database|queue|cache|http|grpc"
```
...
Produce a backend diagnosis with targeted fixes.
