---
name: ci-failure
description: Use when Codex needs help with triaging CI failures and isolating the first actionable error.
---

# CI Failure Lab Skill

## Purpose

Use this skill when Codex needs to handle triaging CI failures and isolating the first actionable error. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
rg "error|failed|Traceback|Cannot find"
```
...
Produce a CI root-cause note and fix path.
