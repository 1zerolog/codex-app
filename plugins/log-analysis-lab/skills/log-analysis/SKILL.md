---
name: log-analysis
description: Use when Codex needs help with turning noisy logs into a timeline and likely causes.
---

# Log Analysis Lab Skill

## Purpose

Use this skill when Codex needs to handle turning noisy logs into a timeline and likely causes. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
```bash
rg "ERROR|WARN|Traceback|Exception|panic|timeout"
rg "request|trace|correlation|job"
```
...
Produce a log timeline and next checks.
