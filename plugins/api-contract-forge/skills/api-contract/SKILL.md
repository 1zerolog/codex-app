---
name: api-contract
description: Use when Codex needs help with reviewing and shaping API contracts before implementation.
---

# API Contract Forge Skill

## Purpose

Use this skill when Codex needs to handle reviewing and shaping API contracts before implementation. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
```bash
rg "openapi|swagger|router|endpoint|schema"
rg "status\(|json\(|throw|error|pagination"
```
...
Produce an API contract review and implementation checklist.
