...
description: Use when Codex needs help with planning rollback for risky releases, migrations, and config changes.
---
...
Use this skill when Codex needs to handle planning rollback for risky releases, migrations, and config changes. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
```bash
git log --oneline -20
rg "rollback|revert|restore|backup" docs README.md scripts
```
