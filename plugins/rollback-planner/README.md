...
Rollback Planner is a Codex Labkit plugin for planning rollback for risky releases, migrations, and config changes. It packages a focused workflow as a bundled skill so Codex can approach this category of work consistently.

## Why This Plugin Exists

Codex works best when broad requests are converted into clear investigation, action, and verification loops. This plugin gives Codex a repeatable path for planning rollback for risky releases, migrations, and config changes, with enough structure to be useful and enough restraint to avoid ceremony.

## Bundled Skill

- `rollback-planner` - the main workflow used by this plugin.

## Good Fits

- The user asks for help with planning rollback for risky releases, migrations, and config changes.
