---
name: config
description: Use when Codex needs help with auditing configuration files, defaults, and environment variables.
---

# Config Sentinel Skill

## Purpose

Use this skill when Codex needs to handle auditing configuration files, defaults, and environment variables. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
```bash
rg "process.env|os.environ|dotenv|settings|config"
find . -maxdepth 3 -name "*.env*" -o -name "*config*"
```
...
Produce a configuration risk report.
