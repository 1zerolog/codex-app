---
name: schema
description: Use when Codex needs help with reviewing schemas and compatibility boundaries.
---

# Schema Guardian Skill

## Purpose

Use this skill when Codex needs to handle reviewing schemas and compatibility boundaries. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
```bash
rg "schema|zod|jsonschema|graphql|type|interface"
rg "migration|contract|validate"
```
...
Produce a schema compatibility report.
