...
description: Use when Codex needs help with improving frontend layout, state coverage, and product feel.
---

# Frontend Polish Lab Skill

## Purpose

Use this skill when Codex needs to handle improving frontend layout, state coverage, and product feel. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
```bash
rg "className|style|css|tailwind|loading|empty|error"
rg "button|input|select|dialog|modal"
```
...
Produce a frontend polish checklist and fixes.
