...
description: Use when Codex needs help with upgrading dependencies with scoped risk and verification.
---

# Dependency Upgrade Studio Skill

## Purpose

Use this skill when Codex needs to handle upgrading dependencies with scoped risk and verification. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
```bash
rg --files | rg "package-lock|pnpm-lock|yarn.lock|requirements|poetry.lock|uv.lock"
rg "dependencies|devDependencies|requires"
```
...
```markdown
## Dependency Upgrade Studio Result
...
Produce a staged dependency upgrade plan.
