...
description: Use when Codex needs help with planning logs, metrics, traces, and alerts for a change.
---

# Observability Planner Skill

## Purpose

Use this skill when Codex needs to handle planning logs, metrics, traces, and alerts for a change. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
```bash
rg "log|metric|trace|span|telemetry|alert"
rg "logger|prometheus|opentelemetry|sentry"
```
