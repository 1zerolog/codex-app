...
description: Use when Codex needs help with reviewing performance budgets and obvious bottlenecks.
---

# Performance Budget Lab Skill

## Purpose

Use this skill when Codex needs to handle reviewing performance budgets and obvious bottlenecks. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
```bash
rg "lazy|memo|cache|index|query|fetch"
rg "bundle|analyze|lighthouse|profile" package.json
```
...
Produce a performance budget report.
