...
description: Use when Codex needs help with reviewing cache keys, invalidation, and stale data risks.
---

# Cache Invalidation Lab Skill

## Purpose

Use this skill when Codex needs to handle reviewing cache keys, invalidation, and stale data risks. The goal is to create a dependable workflow that turns a broad request into inspection, action, validation, and a concise user-facing result.
...
```bash
rg "cache|ttl|invalidate|memo|redis|localStorage"
rg "etag|last-modified|revalidate"
```
